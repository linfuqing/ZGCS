...
#region TABLE
        ...
        public static readonly int[][] triangleTable = new int[256][] {
            null,
            new int[] {0, 8, 3},
            new int[] {0, 1, 9},
            new int[] {1, 8, 3, 9, 8, 1},
        ...
        public static Vector3Int[] edgeOffsets = new Vector3Int[]
        {
        ...
        public static Vector3Int[] edgeToVertexIndices = new Vector3Int[]
        {
            new Vector3Int(0, 1, 0), //(0, 0, 0)
            new Vector3Int(1, 2, 2), //(1, 0, 0)
            new Vector3Int(2, 3, 0), //(0, 0, 1)
            new Vector3Int(3, 0, 2), //(0, 0, 0)
            new Vector3Int(4, 5, 0), //(0, 1, 0)
            new Vector3Int(5, 6, 2), //(1, 1, 0)
            new Vector3Int(6, 7, 0), //(0, 1, 1)
            new Vector3Int(7, 4, 2), //(0, 1, 0)
            new Vector3Int(0, 4, 1), //(0, 0, 0)
            new Vector3Int(1, 5, 1), //(1, 0, 0)
            new Vector3Int(2, 6, 1), //(1, 0, 1)
            new Vector3Int(3, 7, 1) //(0, 0, 1)
        };

        public static Vector3Int[] vertexIndexToDelta = new Vector3Int[8]
        {
            new Vector3Int(0, 0, 0),
            new Vector3Int(1, 0, 0),
            new Vector3Int(1, 0, 1),
        ...
        private Dictionary<Vector3Int, int> __indices;
        
        public void Add(Vector3Int position, float isovalue)
        {
            ...
            __blocks[position] = new Block(isovalue);

            if (__indices == null)
                __indices = new Dictionary<Vector3Int, int>();

            Vector3Int key;
            int index;
            for (int i = 0; i < 8; ++i)
            {
                key = position - vertexIndexToDelta[i];
                if (!__indices.TryGetValue(key, out index))
                    index = 0;
        ...
        public Mesh ToFlatMesh(Mesh mesh, Vector3 offset, Vector3 scale)
        {
            scale *= 0.5f;

            int i, numEdgeIndices, index, edgeIndex;
            Vector3Int position, key, vertexIndices;
            int[] edgeIndices;
            List<Vector3> vertices = null;
            Dictionary<Vector3Int, Vector3> vertexMap;
            Dictionary<Vector3Int, Vector3>[] vertexTable = null;
            foreach(KeyValuePair<Vector3Int, int> pair in __indices)
            {
                position = pair.Key;

                index = pair.Value;

                edgeIndex = edgeTable[index];
                for(i = 0; i < 12; ++i)
                {
                    if((edgeIndex & (1 << i)) != 0)
                    {
                        vertexIndices = edgeToVertexIndices[i];

                        if (vertexTable == null)
                            vertexTable = new Dictionary<Vector3Int, Vector3>[3];

                        vertexMap = vertexTable[vertexIndices.z];
                        if (vertexMap == null)
                        {
                            vertexMap = new Dictionary<Vector3Int, Vector3>();

                            vertexTable[vertexIndices.z] = vertexMap;
                        }

                        key = position + edgeOffsets[i];
                        if (!vertexMap.ContainsKey(key))
                            vertexMap[key] = Vector3.Scale(
                                __VertexInterp(
                                    vertexIndexToDelta[vertexIndices.x] + position, 
                                    vertexIndexToDelta[vertexIndices.y] + position), scale) + offset;
                    }
                }

                edgeIndices = triangleTable[index];
                numEdgeIndices = edgeIndices == null ? 0 : edgeIndices.Length;
                for(i = 0; i < numEdgeIndices; i += 3)
                {
                    if (vertices == null)
                        vertices = new List<Vector3>();

                    edgeIndex = edgeIndices[i + 0];
                    vertices.Add(vertexTable[edgeToVertexIndices[edgeIndex].z][position + edgeOffsets[edgeIndex]]);

                    edgeIndex = edgeIndices[i + 1];
                    vertices.Add(vertexTable[edgeToVertexIndices[edgeIndex].z][position + edgeOffsets[edgeIndex]]);

                    edgeIndex = edgeIndices[i + 2];
                    vertices.Add(vertexTable[edgeToVertexIndices[edgeIndex].z][position + edgeOffsets[edgeIndex]]);
                }
            }
        ...
        private Vector3 __VertexInterp(Vector3Int x, Vector3Int y)
        {
            Block block;
            float isovalueX = __blocks.TryGetValue(x, out block) ? block.isovalue : 1.0f;
            float isovalueY = __blocks.TryGetValue(y, out block) ? block.isovalue : 1.0f;
            
            if (Mathf.Approximately(0.0f, isovalueX))
                return x;

            if (Mathf.Approximately(0.0f, isovalueY))
                return y;

            if (Mathf.Approximately(isovalueX, isovalueY))
                return x;

            return Vector3.Lerp(x, y, isovalueX / (isovalueX - isovalueY));
        }
    }
}